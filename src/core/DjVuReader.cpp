...
qDebug() << "DjVuReader::readImage " + filename;
    QMutexLocker locker(&_mutex);
    ddjvu_context_t *context = ddjvu_context_create("scan_tailor_universal-thumb");
    if (!context) {
        return res;
    }


//    ddjvu_cache_clear(_context);

#if DDJVUAPI_VERSION >= 19
    QByteArray b = filename.toUtf8();
    ddjvu_document_s * document = ddjvu_document_create_by_filename_utf8(context, b, true);
#else
    QByteArray b = QFile::encodeName(filename);
    ddjvu_document_s * document = ddjvu_document_create_by_filename(context, b, true);
#endif

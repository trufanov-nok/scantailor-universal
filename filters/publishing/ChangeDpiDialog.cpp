...
ChangeDpiDialog::ChangeDpiDialog(QWidget* parent, Dpi const& dpi):	QDialog(parent)
{
	setupUi(this);
...
void publishing::ChangeDpiDialog::on_buttonBox_clicked(QAbstractButton *button)
{
    if (button && button == static_cast<QAbstractButton*>(buttonBox->button(QDialogButtonBox::Ok))) {
        if (validate()) {
            accept();
        }
    } else {

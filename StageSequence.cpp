...
:	m_ptrFixOrientationFilter(new fix_orientation::Filter(page_selection_accessor)),
    ...
    m_ptrPublishingFilter(new publishing::Filter(pages, page_selection_accessor))
{
    ...
    m_publishingFilterIdx = m_filters.size();
    m_filters.push_back(m_ptrPublishingFilter);
}
